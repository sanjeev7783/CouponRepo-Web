...
export function PrashadManager({ userEmail }: PrashadManagerProps) {
  ...
  const handleEdit = (prashad: Prashad) => {
    setFormData({
      ...
      price_in_cents: prashad.price_in_cents || 0,
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  required
                  value={formData.price_in_cents === 0 ? '' : (formData.price_in_cents / 100).toString()}
                  onChange={(e) => {
                    const value = e.target.value
                    if (value === '') {
                      setFormData({ ...formData, price_in_cents: 0 })
                    } else {
                      const cents = Math.round(parseFloat(value) * 100)
                      setFormData({ ...formData, price_in_cents: isNaN(cents) ? 0 : cents })
                    }
                  }}
                  placeholder="0.00"
                />
