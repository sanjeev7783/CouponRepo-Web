...
export function CouponList({ coupons, user }: CouponListProps) {
  ...
  const updateQuantity = (couponId: string, delta: number) => {
    setCart((prev) => {
      return prev
        .map((item) => (item.coupon.id === couponId ? { ...item, quantity: Math.max(0, item.quantity + delta) } : item))
        .filter((item) => item.quantity > 0)
    })
  }

  const totalItems = cart.reduce((sum, item) => sum + item.quantity, 0)
  const totalPrice = cart.reduce((sum, item) => sum + item.coupon.price_in_cents * item.quantity, 0)
                      ...
                      {cartItem ? (
                        <div className="flex items-center gap-3 w-full">
                          <Button
                            size="icon"
                            variant="outline"
                            onClick={() => updateQuantity(coupon.id, -1)}
